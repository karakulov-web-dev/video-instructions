var player_cond = 0,
  player = {
    ...
    timer_check_start_player_action: function() {
      ...
      switch (current.playMode) {
        ...
        case "random":
          if (
            player.timer_check_start_player_count <
            player.timer_check_start_player_max_count
          ) {
            if (player_cond != 1) {
              loading.show();
              player.playingPreparation(player.obj);
          ...
          if (player_cond != 1) {
            loading.show();
            player.playingPreparation(
              current.globalObj[random(current.globalObj.length)]
            );
          }
          break;
        case "list":
    ...
    playingPreparation: function(obj) {
      ...
      player.obj = obj;
      var url = player.obj.server;
      //$.getJSON("http://video.rikt.ru/video3/stb/stb_url.php?callback=?", function(data){
      //var url = data.url + server;
      ...
      player.timer_check_start_player_obj = setTimeout(
        "player.timer_check_start_player_action();",
        player.timer_check_start_player_interval
      );
    },
    /**
    ...
    play: function(arr) {
      ...
      isset_qualites += " - - - -";
      //log(isset_qualites);
      for (var i = 0; i < prioritets[current.priority].length; i++) {
        // check all current priorites
      ...
      byID("filename").innerHTML =
        player.obj.title.length > 35
          ? player.obj.title.substr(0, 34) + "…"
          : player.obj.title; // cut move name
      ...
      time_obj.hours = Math.floor(time / 3600);
      time_obj.minuts = Math.floor((time - time_obj.hours * 3600) / 60);
      time_obj.seconds = time - time_obj.hours * 3600 - time_obj.minuts * 60;
      byID("time_act").innerHTML = "00:00:00";
      byID("time_total").innerHTML =
        "/ " +
        (time_obj.hours < 10 ? "0" + time_obj.hours : time_obj.hours) +
        ":" +
        (time_obj.minuts < 10 ? "0" + time_obj.minuts : time_obj.minuts) +
        ":" +
        (time_obj.seconds < 10 ? "0" + time_obj.seconds : time_obj.seconds);
    ...
    play_clip: function(url) {
      byID("shell").style.display = "none";
      player.current_step = 30;
      player_cond = 1;
      current.layer = layers.PLAYER;
      ...
      if (current.subtitles == "sub_on") {
        stb.SetSubtitleLangs("rus", "");
      }
      if (current.subtitles == "sub_on_eng") {
        stb.SetSubtitleLangs("eng", "");
      }
      ...
      switch (event) {
        ...
        case 2: // obtained information about audio and video tracks of media content
          ...
          /*
              ...
              var first_sub_lang = array_of_objects[0].lang[0];  //Обычно русские сабы располагаются в первой дороге. 
            */
          ...
          /*
             if (current.subtitles=='sub_on' && first_sub_lang=='rus') {
                stb.SetSubtitles(1);
              } else if (current.subtitles=='sub_on_eng') {
               stb.SetSubtitles(1);
             } else {
               stb.SetSubtitles(0);
             }
        ...
        case "up":
          for (i = player.steps.length - 1; i >= 0; i--) {
            if (player.steps[i] == player.current_step) {
              ...
              byID("interval_input").innerHTML =
                lang.intervals[player.current_step];
              break;
            }
          }
          break;
    ...
    every_sec: function() {
      ...
      t_time_obj.seconds =
        t_time - t_time_obj.hours * 3600 - t_time_obj.minuts * 60;
      ...
      byID("time_act").innerHTML =
        (t_time_obj.hours < 10 ? "0" + t_time_obj.hours : t_time_obj.hours) +
        ":" +
        (t_time_obj.minuts < 10 ? "0" + t_time_obj.minuts : t_time_obj.minuts) +
        ":" +
        (t_time_obj.seconds < 10
          ? "0" + t_time_obj.seconds
          : t_time_obj.seconds);
    ...
    rewind_afterPause: function(to) {
      ...
      byID("conditiion").style.marginLeft =
        Math.floor((player.where / len) * player.scrolbar_width) + "px";
      player.timer = setInterval(
        player.rewind_every_100_ms,
        player.timer_iv_step
      );
    },
    ...
    rewind: function(to) {
      ...
      if (to > 0) {
        ...
        if (rest > player.current_step) {
          ...
          player.where = Math.floor(
            ((len - player.where) * 9) / 10 + player.where
          );
        }
      } else {
      ...
      if (byID("main").style.display == "block") {
        ...
        switch (player.n_where.length) {
          ...
          case 3:
            byID("time_act").innerHTML =
              "00:0" +
              player.n_where.substr(0, 1) +
              ":" +
              player.n_where.substr(1, 2);
          ...
          case 4:
            byID("time_act").innerHTML =
              "00:" +
              player.n_where.substr(0, 2) +
              ":" +
              player.n_where.substr(2, 2);
            sec = parseInt(player.n_where.substr(0, 2));
            min = parseInt(player.n_where.substr(2, 2));
            player.n_where_sec =
              sec > 60 ? 58 : sec + (min > 60 ? 58 : min) * 60;
            break;
          case 5:
            byID("time_act").innerHTML =
              "0" +
    ...
    pressVolume: function(direction) {
      ...
      player.volume_timer = setInterval(
        player.volume_every_100_ms,
        player.volume_time_step
      );
    ...
    displayModeChange: function() {
      if (player.dmc_var) {
        clearTimeout(player.dmc_var);
      }
      player.dmc_var = null;
      aspect_current = (aspect_current + 1) % 4;
      stb.SetAspect(aspects[aspect_current].mode);
      byID("screenAspect").style.backgroundImage =
        "url(" + aspects[aspect_current].img + ")";
      byID("screenAspect").style.backgroundPosition =
        "0 -" + aspect_current * 41 + "px";
      byID("screenAspect").style.display = "block";
      player.dmc_var = setTimeout(function() {
        byID("screenAspect").style.display = "none";
      }, 3000);
    }
  },
  workWithItems = {
   ...
   */
      ...
      byID("arrow_left").style.display =
        current.page - 1 < 1 ? "none" : "block"; // set visibility of arrow to up
      for (
        var i = (current.page - 1) * items.atPage;
        i < current.page * items.atPage;
        i++
      ) {
        if (isset(current.globalObj[i])) {
          byID("frame").innerHTML +=
            ...
            '       <p class="title">' +
            (function() {
              return current.globalObj[i].title.substr(0, 55);
            })() +
            "</p>" +
            "\n" +
            '       <p class="film_time">' +
            current.globalObj[i].category +
            "</p>" +
            "\n" +
            "   </div>" +
            "\n" +
      ...
      byID("arrow_right").style.display =
        current.page == Math.ceil(request.totalItems / items.atPage)
          ? "none"
          : "block";
      this.focusMovie.apply(this);
    },
    /**
    ...
    focusMovie: function() {
      ...
      if (isset(o2)) {
        if (isset(o1)) {
          o1.className = o1.className.replace(
            new RegExp("(^|\\s)" + n_class + "(\\s|$)"),
            " "
          );
        }
        current.obj = n_o;
  ...
  settings = {
    ...
    caption: function(server_id) {
      current.layer = layers.CAPTION;
      byID("caption").style.display = "block";
      try {
        $.getJSON(
          "http://video.rikt.ru/video3/stb/stb_mag200_caption_full.php?id=" +
            id_f.toString() +
            "&server=" +
            server_id.toString() +
            "&callback=?",
          function(data) {
            ...
            time_obj.minuts = Math.floor((time - time_obj.hours * 3600) / 60);
            time_obj.seconds =
              time - time_obj.hours * 3600 - time_obj.minuts * 60;
            dur =
              (time_obj.hours < 10 ? "0" + time_obj.hours : time_obj.hours) +
              ":" +
              (time_obj.minuts < 10 ? "0" + time_obj.minuts : time_obj.minuts) +
            ...
            dur_txt =
              time > 0
                ? '<br><label style="color:#28b6fc">Продолжительность: </label>' +
                  dur
                : "";
            kp_rating_txt = kp_rating
              ? '<br><label style="color:#28b6fc">Рейтинг:</label> Кинопоиск ' +
                kp_rating
              : "";
            imdb_rating_txt = imdb_rating ? " / IMDB " + imdb_rating : "";
            produser = produser
              ? '<br><label style="color:#28b6fc">Режиссер: </label>' + produser
              : "";
            actors = actors
              ? '<br><br><label style="color:#28b6fc">В ролях: </label>' +
                actors
              : "";
            genre = genre
              ? '<br><label style="color:#28b6fc">Жанр: </label>' + genre
              : "";
            year = year
              ? '<br><br><label style="color:#28b6fc">Год: </label>' + year
              : "";

            document.getElementById("modalcaption_text").innerHTML =
              description +
              year +
              genre +
              dur_txt +
    ...
    options: {
      menuItems: ["PlayModes", "Quality", "SortModes", "Subtitles"],
      subMenuItems: [
        ["single", "repeat", "list", "random"],
        ["desc", "asc"],
        ["name", "year", "views", "add"],
        ["sub_on", "sub_on_eng", "sub_off"]
      ],
      currentLevel: -1,
      menuItem: 0,
      subMenuItem: 0,
      show: function() {
        ...
        byID(
          "menu" + settings.options.menuItems[settings.options.menuItem]
        ).className = "active";
      },
      hide: function() {
        current.layer = layers.BASE;
          ...
          case keys.DOWN:
            ...
            switch (settings.options.currentLevel) {
              case 0:
                ...
                settings.options.menuItem =
                  Math.abs(settings.options.menuItem + shift) %
                  settings.options.menuItems.length;
                byID(
                  "menu" + settings.options.menuItems[settings.options.menuItem]
                ).className = "active";
                break;
              case 1:
                for (
                  var i = 0;
                  i <
                  settings.options.subMenuItems[settings.options.menuItem]
                    .length;
                  i++
                ) {
                  byID(
                ...
                settings.options.subMenuItem =
                  (settings.options.subMenuItem + shift) %
                  settings.options.subMenuItems[settings.options.menuItem]
                    .length;
                settings.options.subMenuItem =
                  settings.options.subMenuItem >= 0
          ...
          case keys.OK:
            switch (settings.options.currentLevel) {
              case 0:
                byID(
                  "subMenu" +
                    settings.options.menuItems[settings.options.menuItem]
                ).style.display = "block";
                for (
                  var i = 0;
                  i <
              ...
              case 1:
                ...
                var sm = document
                  .getElementById("footer_menu_settings")
  ...
  categoryes = {
    ...
    getOne: function(cur) {
      return cur % categorias.length >= 0
        ? cur % categorias.length
        : categorias.length + (cur % categorias.length);
    }
  };
