    ...
    */
    "play": function(arr) {
      ...
      "play_clip": function(url) {
        ...
        byID('filename').innerHTML = player.obj.title.length > 35 ? player.obj.title.substr(0, 34) + "..." : player.obj.title;  
        byID('time_act').innerHTML = '00:00:00';    
        byID('interval_input').innerHTML = lang.intervals[player.current_step]; 
        ...
        for(var i=(current.page - 1) * items.atPage; i <  current.page * items.atPage; i++) {
            if(isset(current.globalObj[i])) {
                ...
                byID("frame").innerHTML += '' +
                    ...
                    '       <p class="film_time">'+ current.globalObj[i].category + '</p>' + "\n" +
                    '   </div>' + "\n" +
                    '</div>';
            }
        }
        byID("arrow_right").style.display = (current.page == Math.ceil(request.totalItems/items.atPage)) ? "none" : "block"; 
...
settings = {
    ...
    */
    "cats_start": function() {
        ...
        (current.feed + '&start-index=' + request.startIndex.toString() + '&max-results=' + request.itemsPerRequest.toString() + '&order-by=' + current.sortMode+ '&sort-order=' + current.priority,
            'rebuildCurrentGlobalObj');  
  ...
  try {
     $.getJSON("http://video.rikt.ru/video3/stb/stb_mag200_caption_full.php?id="+id_f.toString()+"&server="+server_id.toString()+"&callback=?", function(data){
     genre=data.genre;
     title=data.title;
     year=data.year;
     ...
     imdb_rating = data.imdb_rating;     
             ...
             var time = duration;
               ...
               year = year?'<br><br><label style="color:#28b6fc">Год: </label>'+year:'';
     
     document.getElementById('modalcaption_text').innerHTML = description+year+genre+dur_txt+kp_rating_txt+imdb_rating_txt+actors+produser;
    });
     }
      catch(err) {
    }
    ...
    */
    ...
    "options":{
        ...
        "subMenuItems":[["single","repeat","list","random"],["desc","asc"],["name","year","views","add"],["sub_on","sub_on_eng","sub_off"]],
        "currentLevel":-1,
        "menuItem":0,
        "subMenuItem":0,
        "show":function(){
