      ...
      switch (code) {
        case keys.EXIT:
          ...
          if (folder == 1) {
            ...
            current.obj = 0;
            current.page = curr_page_folder;
            workWithItems.shift = parseInt(item_shift_folder);
            itemsPerRequest = request.itemsPerRequest * current.page;
            loading.show();

            getData(
              current.feed +
                "&start-index=1&max-results=" +
                itemsPerRequest.toString() +
                "&order-by=" +
                current.sortMode +
                "&sort-order=" +
                current.priority,
        ...
        case keys.UP:
          if (current.obj - (current.page - 1) * items.atPage >= items.atLine) {
            ...
            if (
              current.obj - (current.page - 1) * items.atPage < items.atLine &&
              current.obj > items.atLine
            ) {
              current.page--;
              workWithItems.shift = -items.atLine;
              workWithItems.drawBoxes();
        ...
        case keys.RIGHT:
          if (current.obj + 1 == current.page * items.atPage) {
            if (
              ...
              if (folder == 1) {
                getData(
                  "http://video.rikt.ru/video3/stb/stb_mag200_folder_full_2.php?id=" +
                    id_f.toString() +
                    "&start-index=" +
        ...
        case keys.PAGE_NEXT:
          if (
            (current.page + 1) * items.atPage <= current.globalObj.length ||
            request.totalItems == current.globalObj.length - 1
          ) {
        ...
        case keys.OK:
          ...
          if (current.globalObj[current.obj].id_folder == 0) {
            player.playingPreparation(current.globalObj[current.obj]);
          } else {
            folder = 1;
            curr_page_folder = current.page;
            item_shift_folder = current.obj;
            current.globalObj = new Array();
            current.obj = 0;
            current.page = 1;
        ...
        case keys.CHANNEL_NEXT:
          ...
          if (current.obj + 1 == current.page * items.atPage) {
            if (
              ...
              setTimeout(function() {
                var id = byID("frame").getElementsByClassName("active")[0]
                  .title;
                for (var i in current.globalObj) {
                  if (current.globalObj[i].id == id) {
                    //log('Set action id : ' + id);
